...
import NavbarTwoColor from '../components/NavbarTwoColor';
import YourSVGTwoColors from '../components/YourSVGTwoColors';
import NewWaveSVGTwoColors from '../components/NewWaveSVGTwoColors';
import BrandSVGTwoColors from '../components/BrandSVGTwoColorsPage';
import Harmony3DTextTwoColors from '../components/Harmony3DTextTwoColors';
...
const TwoColorsPage = () => {
  ...
  const [isAdditionalColorsLocked, setIsAdditionalColorsLocked] = useState([false, false]);
  ...
  const [isExtraPickerVisible, setExtraPickerVisible] = useState(false);
  const [copySuccess, setCopySuccess] = useState('');

  const basePickerRef = useRef(null);
  const diamondPickerRef = useRef(null);
  const extraPickerRef = useRef(null);

  const fetchInitialColors = async () => {
    try {
      setProgress(30); // Start the loading bar
      const colors = await getColors(1); // Fetch colors with one accent
      const initialBaseColor = rgbArrayToHex(colors.mainColor);
      const initialAdditionalColors = [rgbArrayToHex(colors.secondaryColor), rgbArrayToHex(colors.accentColor1)];
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (extraPickerRef.current && !extraPickerRef.current.contains(event.target)) {
  ...
  const onGenerate = async () => {
    ...
    try {
      ...
      const newAdditionalColors = [
        isAdditionalColorsLocked[0] ? additionalColors[0] : rgbArrayToHex(colors.secondaryColor),
        isAdditionalColorsLocked[1] ? additionalColors[1] : rgbArrayToHex(colors.accentColor1)
      ];
  ...
  const handleExtraColorChange = (color) => {
  ...
  const extraColor = additionalColors[1];
      ...
      <div className="flex-grow relative">
        ...
        <div style={{ position: 'relative', backgroundColor: baseColor }}>
          <div className="new-wave-svg-container">
            <NewWaveSVGTwoColors baseColor={baseColor} diamondColor={diamondColor} extraColor={extraColor} />
          </div>
        </div>
        ...
        <section
          ...
          style={{ backgroundColor: baseColor }}
        >
          <YourSVGTwoColors baseColor={baseColor} diamondColor={diamondColor} extraColor={extraColor} /> {/* Render your additional SVG component here */}
        </section>
        ...
        >
          <Harmony3DTextTwoColors baseColor={baseColor} diamondColor={diamondColor} extraColor={extraColor} />
        </section>
        <section
        ...
        >
          ...
          <div className="flex w-[825px] h-[425px] relative z-10 custom-shadow" style={{ borderRadius: '25px' }}>
                ...
                <div className="flex flex-col items-center">
                  <div className="icon-container mb-2" style={{ color: isColorDark(diamondColor) ? 'white' : 'black' }}>
                    ...
                    <FontAwesomeIcon 
                      ...
                      onClick={() => setIsAdditionalColorsLocked([!isAdditionalColorsLocked[0], isAdditionalColorsLocked[1]])}
                      className="icon"
            ...
            >
              <div className="absolute bottom-28 text-center">
                <div className="flex flex-col items-center">
                  <div className="icon-container mb-2" style={{ color: isColorDark(extraColor) ? 'white' : 'black' }}>
                    <FontAwesomeIcon 
                      icon={faCopy} 
                      onClick={() => copyToClipboard(extraColor)}
                      className="icon"
                      title="Copy"
                    />
                    <FontAwesomeIcon 
                  ...
                  <p className="text-1xl font-semibold no-select" style={{ color: isColorDark(extraColor) ? 'white' : 'black' }}>{ntc.name(extraColor)[1]}</p>
                  {isExtraPickerVisible && (
                    <div ref={extraPickerRef} style={{ position: 'absolute', top: '30%', right: 'calc(50% + 206px)', transform: 'translate(50%, -50%)', zIndex: '10', background: '#fff', padding: '10px', borderRadius: '8px', boxShadow: '0 0 10px rgba(0, 0, 0, 0.1)' }}>
                      <SketchPicker 
                        color={extraColor} 
                        onChange={handleExtraColorChange} 
                        width="200px" 
                        styles={{ default: { picker: { boxShadow: 'none', border: '1px solid #d3d3d3', borderRadius: '8px' } } }}
                      />
                      <button
