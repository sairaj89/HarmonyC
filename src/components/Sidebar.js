...
import { faEye, faEyeSlash, faDiceD6, faPager, faImage, faInfoCircle, faBars, faTimes } from '@fortawesome/free-solid-svg-icons'; // Added faBars and faTimes for hamburger icon
...
const Sidebar = ({ setSelectedOption }) => {
  ...
  const [isVisible, setIsVisible] = useState(false); // Visibility state for mobile
  ...
  // New function to toggle sidebar visibility for mobile
  const toggleSidebar = () => {
  ...
  return (
    <>
      ...
      <div className="md:hidden flex justify-between items-center bg-blue-900 px-4 py-2 text-white">
        <button onClick={toggleSidebar} className="text-2xl focus:outline-none">
          <FontAwesomeIcon icon={isVisible ? faTimes : faBars} />
        </button>
        <h1 className="text-lg font-bold">Harmony</h1>
      </div>

      {/* Sidebar with visibility toggle */}
      <div
        className={`sidebar bg-white text-gray-800 h-full fixed top-0 z-30 transition-transform duration-300 ${isVisible ? 'translate-x-0' : '-translate-x-full'
          } md:translate-x-0 md:w-64 sf-pro-font ${isPinned ? 'pinned' : ''}`}
        style={{ width: '16rem' }}
      >
        ...
        <div className="p-4 flex-grow overflow-y-auto scrollable-content">
          <div
            className={`relative mb-4 section ${selectedOption.includes('color') || selectedOption === 'Brand' ? 'bg-off-white highlighted-section' : ''
              }`}
          >
            <button
              onClick={handleBrandToggle}
              className={`flex justify-between items-center w-full text-left px-4 py-2 rounded-md hover:text-black hover:opacity-100 ${selectedOption.includes('color') ? 'text-black opacity-100' : 'opacity-70'
                }`}
            >
            ...
            >
              <div className="mt-2 w-full">
                <button
                  ...
                  className={`flex justify-between items-center w-full text-left px-4 py-2 hover:text-black hover:opacity-100 ${selectedOption === '1 color' ? 'neon-text' : 'opacity-70'
                    }`}
                ...
                <button
                  ...
                  className={`flex justify-between items-center w-full text-left px-4 py-2 hover:text-black hover:opacity-100 ${selectedOption === '2 colors' ? 'neon-text' : 'opacity-70'}`}
                  style={{ marginBottom: '0.5rem' }}
                >
                ...
                <button
                  ...
                  className={`flex justify-between items-center w-full text-left px-4 py-2 hover:text-black hover:opacity-100 ${selectedOption === '3 colors' ? 'neon-text' : 'opacity-70'}`}
                >
          ...
          <div
            className={`relative mb-4 section ${selectedOption.includes('tone') || selectedOption === 'Website' ? 'bg-off-white highlighted-section' : ''
              }`}
          >
            <button
              onClick={handleWebsiteToggle}
              className={`flex justify-between items-center w-full text-left px-4 py-2 rounded-md hover:text-black hover:opacity-100 ${selectedOption.includes('tone') ? 'text-black opacity-100' : 'opacity-70'}`}
            >
            ...
            >
              <div className="mt-2 w-full">
                <button
                  ...
                  className={`flex justify-between items-center w-full text-left px-4 py-2 hover:text-black hover:opacity-100 ${selectedOption === '1 tone' ? 'neon-text' : 'opacity-70'}`}
                  style={{ marginBottom: '0.5rem' }}
                >
                ...
                <button
                  ...
                  className={`flex justify-between items-center w-full text-left px-4 py-2 hover:text-black hover:opacity-100 ${selectedOption === '2 tones' ? 'neon-text' : 'opacity-70'}`}
                >
          ...
          <div
            className={`relative mb-4 section ${selectedOption === 'About' ? 'bg-off-white highlighted-section' : ''}`}
          >
            <button
              onClick={() => handleOptionSelect('About')}
              className={`flex justify-between items-center w-full text-left px-4 py-2 rounded-md hover:text-black hover:opacity-100 ${selectedOption === 'About' ? 'text-black opacity-100' : 'opacity-70'}`}
            >
