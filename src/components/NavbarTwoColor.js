...
const NavbarTwoColor = ({
  ...
  isAdditionalColorsLocked,
  setIsAdditionalColorsLocked,
  ...
  const toggleLock = (index) => {
    if (index === 0) {
      ...
      setIsAdditionalColorsLocked((prevLocks) => {
  ...
  return (
    <header className="flex items-center h-16 px-4 shadow fixed top-0 w-full z-20" style={{ backgroundColor: '#07252d' }}>
      ...
      <div className="flex items-center space-x-4" id='nv2'>
        <ColorSelector
        ...
        {additionalColors.map((color, index) => (
          <ColorSelector
            ...
            showPicker={showPicker}
            togglePicker={togglePicker}
            isLocked={isAdditionalColorsLocked[index]}
            toggleLock={() => toggleLock(index + 1)}
            pickerRef={index === 0 ? diamondPickerRef : extraPickerRef}
          />
        ))}
        <div className="border-l border-gray-300 h-8 mx-4" id='br2'></div>
        <button onClick={onBack} className={`back-button ${isBackButtonDisabled ? 'opacity-50 cursor-not-allowed' : ''}`} id='b2' disabled={isBackButtonDisabled}>
...
const ColorSelector = ({
  ...
  return (
    <div className="relative">
      <button
        className={`w-8 h-8 rounded-full border-0 cursor-pointer ${isLocked ? 'bg-opacity-50' : ''} color-button ${isSelected ? 'selected' : ''}`} id='cl2'
      ...
      {isOpen && (
        <div ref={pickerRef} className="absolute z-10 mt-2 bg-white p-3 rounded shadow-md w-48 border border-gray-300 menu-button" id='dd2'>
          <button
