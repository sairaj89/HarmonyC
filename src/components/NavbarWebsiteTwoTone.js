...
const NavbarWebsiteTwoTone = ({
  ...
  isDiamondColorLocked,
  setIsDiamondColorLocked,
  isHarColorLocked,
  setIsHarColorLocked,
  isMonyColorLocked,
  setIsMonyColorLocked,
  ...
  const handleColorChange = (index, color) => {
    if (index === 0) {
      ...
      setAdditionalColors([color, additionalColors[1], additionalColors[2]]);
    } else if (index === 2) {
      setAdditionalColors([additionalColors[0], color, additionalColors[2]]);
    } else if (index === 3) {
      setAdditionalColors([additionalColors[0], additionalColors[1], color]);
    }
  };
  ...
  const toggleLock = (index) => {
    if (index === 0) {
      ...
      setIsDiamondColorLocked(!isDiamondColorLocked);
    } else if (index === 2) {
      setIsHarColorLocked(!isHarColorLocked);
    } else if (index === 3) {
      setIsMonyColorLocked(!isMonyColorLocked);
    }
  };
  ...
  return (
    <header className="flex items-center h-16 px-4 shadow fixed top-0 w-full z-20" style={{ backgroundColor: '#07252d' }}>
      ...
      <div className="flex items-center space-x-4" id='nv5'>
        <ColorSelector
        ...
        {additionalColors.map((color, index) => (
          <ColorSelector
            ...
            togglePicker={togglePicker}
            isLocked={
              index === 0 ? isDiamondColorLocked :
              index === 1 ? isHarColorLocked : isMonyColorLocked
            }
            toggleLock={() => toggleLock(index + 1)}
            pickerRef={
              index === 0 ? diamondPickerRef :
              index === 1 ? harPickerRef : monyPickerRef
            }
          />
        ))}
        <div className="border-l border-gray-300 h-8 mx-4" id='br5'></div>
        <button onClick={onBack} id='b5' className={`back-button ${isBackButtonDisabled ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={isBackButtonDisabled}>
          <ArrowLeftIcon className="h-4 w-4" />
        </button>
        <button onClick={onGenerate} className="ml-4 generate-button" id='bu5'>
          Generate
        </button>
      </div>
    </header>
...
const ColorSelector = ({
  ...
  return (
    <div className="relative">
      <button
        className={`w-8 h-8 rounded-full border-0 cursor-pointer ${isLocked ? 'bg-opacity-50' : ''} color-button ${isSelected ? 'selected' : ''}`} id='cl5'
      ...
      {isOpen && (
        <div ref={pickerRef} className="absolute z-10 mt-2 bg-white p-3 rounded shadow-md w-48 border border-gray-300 menu-button" id='dd5'>
          <button
...
export default NavbarWebsiteTwoTone;
